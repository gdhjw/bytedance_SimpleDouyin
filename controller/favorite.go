...
func ActionFavorite(c *gin.Context) {
	...
	err := dao.ActionFavorite(userId, uint(videoId), uint(actionType))
	if err != nil {
		c.JSON(400, gin.H{
			"status_code": 1,
...
func ListFavorite(c *gin.Context) {
	...
	for _, m := range videoList {
		...
		//isfollowing
		//isfollowing := dao.IsFollowing(userIdHost, m.AuthorId) //参数类型、错误处理
		//isfavorite
		isfavorite := dao.CheckFavorite(userIdHost, m.ID)
		//作者信息
	...
	if err != nil {
		...
		c.JSON(200, gin.H{
			"status_code": 0,
			"status_msg":  "list success",
			"video_list":  videoListNew,
		})
	}
