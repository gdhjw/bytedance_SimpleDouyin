...
func CheckFavorite(uid uint, vid uint) bool {
	var num int64
	db.Table("favorites").Where("user_id = ? and video_id = ? and state = 1", uid, vid).Count(&num)
...
func ActionFavorite(userId uint, videoId uint, actionType uint) (err error) {
	...
	if actionType == 1 {
		...
		rs := db.Table("favorites").Where("user_id = ? and video_id = ?", userId, videoId).First(favoriteExist)
		if rs.Error != nil { // 不存在
			if err := db.Table("favorites").Create(&newFavorite).Error; err != nil { //创建记录
				return err
			}
			...
			if favoriteExist.State == 0 { //state为0-video的favorite_count加1
				db.Table("videos").Where("id = ?", videoId).Update("favorite_count", gorm.Expr("favorite_count + 1"))
				db.Table("favorites").Where("video_id = ?", videoId).Update("state", 1)
			}
		...
		if err := db.Table("favorites").Where("user_id = ? AND video_id = ?", userId, videoId).First(&favoriteCancel).Error; err != nil { //找不到这条记录，取消点赞失败，创建记录
			db.Table("favorites").Create(&favoriteActionCancel)
			return err
		}
		//存在
		if favoriteCancel.State == 1 { //state为1-video的favorite_count减1
			db.Table("videos").Where("id = ?", videoId).Update("favorite_count", gorm.Expr("favorite_count - 1"))
			//更新State
			db.Table("favorites").Where("video_id = ?", videoId).Update("state", 0)
		}
